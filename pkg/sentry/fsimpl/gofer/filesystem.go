...
func (fs *filesystem) resolveLocked(ctx context.Context, rp *vfs.ResolvingPath, ds **[]*dentry) (*dentry, error) {
	...
	for !rp.Done() {
		d.dirMu.Lock()
		next, symlink, err := fs.stepLocked(ctx, rp, d, true /* mayFollowSymlinks */, ds)
		d.dirMu.Unlock()
		if err != nil {
			return nil, err
		}
...
func (fs *filesystem) unlinkAt(ctx context.Context, rp *vfs.ResolvingPath, dir bool) error {
	...
	// Load child if:
	//   - sticky bit: we need to retrieve the child to determine whether
	//     it is allowed.
	//   - InteropModeShared: need to validate that the child exists.
	sticky := atomic.LoadUint32(&parent.mode)&linux.ModeSticky != 0
	if sticky || !parent.cachedMetadataAuthoritative() {
